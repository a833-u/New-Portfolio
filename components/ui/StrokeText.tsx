...
const DEFAULT_TEXT = 'ANSH KANSARA';
...
const StrokeText = ({
  ...
  const dash = Math.max(fontSize * 7, 500);
  ...
  return (
    <span
      ref={rootRef}
      className={`block w-full leading-[0] transition-opacity duration-200 ${box ? 'opacity-100' : 'opacity-0'} ${trigger === 'hover' ? 'cursor-pointer' : ''} ${className}`.trim()}
      style={style}
      role="img"
